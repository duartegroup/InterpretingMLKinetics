...
class Conditions:
    def __init__(self, temperature=25 * C, pressure=1 * bar, solvents=None, ionic_strength=0):
    ...
    def ionic_strength(self):
        return self.__ionic_strength

    @ionic_strength.setter
    def ionic_strength(self, value):
        value = float(value)
        if value < 0:
            raise ValueError('negative ionic strength not possible in this universe')
        self.__ionic_strength = value


class DictToConditions(CIMtoolsTransformerMixin):
    def __init__(self, temperature=None, pressure=None, solvents=None, amounts=None, ionic_strength=None,
                 default_temperature=25 * C, default_pressure=1 * bar,
                 default_first_solvent='water', default_first_amount=1, default_ionic_strength=0):
        ...
        self.default_ionic_strength = default_ionic_strength
    ...
    def transform(self, x):
        ...
        if self.ionic_strength:
            ...
            ionic_strength = [d.get(ionic_strength, self.default_ionic_strength) for d in x]
        else:
            ionic_strength = [self.default_ionic_strength] * len(x)


        return DataFrame([[Conditions(t, p, zip(*s),i)]
                          for t, p, s, i in zip(temperatures, pressures, zip(solvents, amounts), ionic_strength)], columns=['conditions'])

    _dtype = Mapping


class ConditionsToDataFrame(CIMtoolsTransformerMixin):
    def __init__(self, max_solvents=2):
    ...
    def get_feature_names(self):
        ...
        return ['temperature', 'pressure'] + [f'solvent.{x}' for x in range(1, self.max_solvents + 1)] + \
               [f'solvent_amount.{x}' for x in range(1, self.max_solvents + 1)] + ['ionic_strength']
    ...
    def transform(self, x):
        ...
        for c in x:
            ...
            res.append(chain((c.temperature, c.pressure), solvents, amounts, (c.ionic_strength,)))
